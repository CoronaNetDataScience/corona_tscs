...
clean_data <- read.csv("../data/CoronaNet/coronanet_release.csv", stringsAsFactors = FALSE) %>% 
  ...
  filter(date_announced<(today()-days(5)),!is.na(country),is.na(target_other)| target_other=="")
...
clean_data %>% 
  ...
  summarize(`Total Number of Policies`=n(),
            ...
            `Number of Targeted Countries`=ifelse(any(grepl('All countries', target_country)),length(unique(unlist(str_split(target_country, ','))))-1, length(unique(unlist(str_split(target_country, ','))))),
...
In order to participate as an RA in this project, RAs must fill out a form^[See [this link](https://docs.google.com/forms/d/e/1FAIpQLSeybAW0DC0UE1x2EqLiTifVFuSUxqJLGFB8VI4wVCG61tVYKg/viewform) for the form.] in which:
