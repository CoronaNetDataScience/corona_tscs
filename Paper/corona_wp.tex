...
Each policy records at the minimum, the following monadic information : the policy type (\texttt{type}), the name of the country from which a policy orginiates (\texttt{country})\^{}{[}If the policy originates from a province or state, that information is also documented in the \texttt{init\_prov\textquotesingle{}\ variable.\ Future\ versions\ of\ the\ dataset\ will\ also\ include\ information\ on\ whether\ a\ policy\ was\ initiated\ from\ a\ city\ or\ municipality\ (}init\_city\texttt{)\ or\ another\ level\ of\ government\ (}init\_other\texttt{).{]},\ the\ degree\ to\ which\ a\ policy\ must\ be\ complied\ with\ (}compliance\texttt{),\ the\ entity\ enforcing\ the\ policy\ (}enforcer\texttt{),\ and\ the\ date\ a\ policy\ is\ announced\ (}date\_annouced\texttt{),\ implemented\ (}date\_start\texttt{)\ and\ ends\ (}date\_end\texttt{).\^{}{[}Note\ that\ sometimes\ policies\ are\ announced\ without\ a\ pre-determined\ end\ date.\ In\ those\ cases,\ this\ field\ is\ left\ blank{]}\ \ When\ a\ policy\ is\ dyadic\ in\ nature,\ the\ database\ further\ documents\ information\ about\ the\ geographic\ target\ of\ the\ policy\ (}target\_geog\_level\texttt{;}target\_region\texttt{,}target\_country\texttt{)\^{}{[}Future\ versions\ of\ the\ dataset\ will\ also\ include\ information\ about\ if\ the\ target\ was\ a\ province\ or\ state\ (}target\_province\texttt{),\ \ a\ city\ or\ municipality\ (}target\_city\texttt{),\ or\ another\ subnational\ unit\ \ (}target\_other\texttt{,\ e.g.\ county,\ university){]},\ the\ human\ or\ material\ target\ of\ a\ policy\ (}target\_who\_what\texttt{),\ the\ directional\ flow\ of\ the\ policy\ (}target\_direction\texttt{),\ and\ the\ mechanism\ of\ travel\ (}target\_mechanism\texttt{).\ Where\ applicable,\ all\ of\ the\ information\ documented\ above\ is\ also\ provided\ qualitatively\ in\ the}event\_description\texttt{variable.\ Additional\ meta-data\ that\ is\ available\ for\ all\ policies\ include\ when\ the\ record\ entered\ into\ the\ database\ (}record\_date\texttt{)\ and\ a\ link\ for\ the\ informaton\ source\ for\ the\ policy\ (}link`). Table \ref{tab:vardesc} provides an overview of the different variables in our dataset, a brief description of what underyling concept each variable aims to capture, and the values that each variable can take on.\footnote{Note that while Table \ref{tab:vardesc} provides an overview of the version of the dataset we have currently made available, future versions of the dataset will also include more detailed information for some policy categories. For example, among other things, we are also collecting information on the types of `health resources' (e.g.~masks, hospitals, doctors ) and types of `restrictions of non-essential business activities' (e.g.~retail businesses, restaurants/bars). Where applicable, we are also collecting information on the volume of a certain policy (e.g.~the number of masks, hospitals and doctors.)}
