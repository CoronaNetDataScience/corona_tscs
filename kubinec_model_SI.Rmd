...
\begin{align}
Pr(I_{ct}|T=t) \sim Beta(&\alpha_1 + \beta_{O1}\sum_{c=1}^{C} \textbf{1}(a_{ct'}>0) \forall t' \in t'<t + \textbf{1}\beta_{S1} + \\
...
sim_data <- list(time_all=time_points,
                 ...
                 time_outbreak=matrix(scale(c(check1$time_counts)),nrow=nrow(check1$time_counts),
...
obs_data <- left_join(obs_data,time_data,by=c("time_points","num_country")) %>% 
  ...
  mutate(time_points=as.numeric(time_points),
         time_points=as.numeric(scale(time_points))) %>% 
...
naive_model <- stan_glm(cbind(obs_data$cases,obs_data$country_pop-obs_data$cases)~poly(time_points,3) +
                          count_outbreak +
                          suppress*time_outbreak)
