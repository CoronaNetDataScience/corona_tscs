...
In this paper, I show how the model can be applied by measuring the association between U.S. state-level factors and the disease as of March 30th, 2020, including the timing of state of emergency declarations, vote share in the 2016 election for President Donald Trump, gross domestic product (GDP), and health-related factors. I show that as of March 31st the timing of state of emergency declarations and vote share for President Donald Trump do not predict the number of infected individuals, while higher numbers of cardiovascular deaths, lower state income and lower air pollution are positively associated with infection rates.
...
combined <- group_by(combined,state) %>% 
  ...
  mutate_at(c("Cardiovascular Deaths",
              ...
              "gdp",
              "trump",
              "day_emergency"), ~as.numeric(scale(.)))
...
obs_model <- stan_glm(cbind(combined$Difference,floor(combined$population/100) - combined$Difference) ~ I(outbreak_time^2) +
                   I(outbreak_time^3) +
                        `Cardiovascular Deaths`*outbreak_time +
                        `Air Pollution`*outbreak_time +
                        `Dedicated Health Care Provider`*outbreak_time +
                        `Smoking`*outbreak_time +
                        `Population under 18 years`*outbreak_time +
                        `Public Health Funding`*outbreak_time +
                        gdp*outbreak_time +
                        trump*outbreak_time,
                      data=combined,
                 QR=TRUE,
...
over_all_time2 <- group_by(over_all,variable,time_scale_point) %>% 
  summarize(med_est=median(estimate),
            high_est=quantile(estimate,.95),
            low_est=quantile(estimate,.05)) %>% 
  mutate(model="Fully\nIdentified")
...
over_all_sum2 <- group_by(over_all,variable) %>% 
  summarize(med_est=median(estimate),
            high_est=quantile(estimate,.95),
            low_est=quantile(estimate,.05)) %>% 
  mutate(model="Fully\nIdentified")
