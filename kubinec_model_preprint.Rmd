...
just_data <- distinct(select(ungroup(combined),state,day_emergency,population,trump,air="Air Pollution",
                      heart="Cardiovascular Deaths",
                      providers="Dedicated Health Care Provider",
                      young="Population under 18 years",
                      smoking="Smoking",
                      gdp,
                      public_health="Public Health Funding")) %>% arrange(state)

covs <- select(ungroup(just_data),-state,-population) %>% as.matrix
...
plot_data3 <- mcmc_intervals_data(obs_model,pars=c("day_emergency",
                                                   ...
                                                   "day_emergency:outbreak_time",
                                                   "outbreak_time:trump",
                                                   "outbreak_time:`Air Pollution`",
                                                   "outbreak_time:`Cardiovascular Deaths`",
                                                   "outbreak_time:`Dedicated Health Care Provider`",
                                                   "outbreak_time:`Population under 18 years`",
                                                   "outbreak_time:Smoking",
                                                   "outbreak_time:gdp",
                                                   "outbreak_time:`Public Health Funding`")) %>% 
  mutate(supp_type=rep(c(1,2),each=9),
         variable=rep(1:9,2))
...
check_tests <- group_by(combined,state,Smoking) %>% summarize(mean_tests=(mean(tests,na.rm=T)/population[1])*10000)
