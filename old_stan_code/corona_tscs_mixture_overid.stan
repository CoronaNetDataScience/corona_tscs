...
parameters {
  ...
  real world_infect;
                                // must have greater effect on encouraging domestic infection than on 
                                // infection rate based on number of travelers
...
model {
  ...
  for (t in 2:time_all) {
    ...
    tests[,t] ~ beta_binomial(country_pop,mu_tests*phi_raw[1],(1-mu_tests)*phi_raw[1]);
    cases[,t] ~ beta_binomial(tests[,t],mu_cases*phi_raw[2],(1-mu_cases)*phi_raw[2]);
